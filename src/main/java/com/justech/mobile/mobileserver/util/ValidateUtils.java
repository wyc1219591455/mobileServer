...
{
    // 密码验证正则表达式
    public static final String REGULAR_PASSWORD   = "^(?=.*[0-9])(?=.*[a-zA-Z])([^ ]{8,32})$";
    ...
    public static boolean isDateFormat(String checkString, String dateFormat, boolean isRequired)
    {
        ...
        if (!boolNull)
        {
            ...
            if (dateFormat.equalsIgnoreCase("YYYY-MM")) { return matchRegexp(checkString, "^([0-9]{4}-[0-9]{1,2})?$"); }
            if (dateFormat.equalsIgnoreCase("yyyy-MM-dd")) { return matchRegexp(checkString, "^([0-9]{4}-[0-9]{1,2}-[0-9]{1,2})?$"); }
        }
        return true;
    }
