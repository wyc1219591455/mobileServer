...
public class UserRepostitory {
    ...
    @Value("${erp.server.user.identification.url}/api/User/ADLogin")
    private String identyURL;
    @Autowired
     ...
     * @return
     */
    public Map getUser(String key) {
        //TODO
       Optional<JustechUser> optionalJustechUser=justechUserRepository.findById(key);
       if (optionalJustechUser.isPresent()){
         return JSONObject.parseObject(JSON.toJSONString(optionalJustechUser.get()),Map.class);
       }else{
           return null;
       }
    ...
    public UserOfErpEntity getUserDetails(String key) {
        ...
        try {
            userOfErpEntity.setImageUrl(this.getImage(userOfErpEntity.getWorkId()).getPath());
        } catch (Exception e) {
        }
        try {
    ...
    public AccountTo identification(String userName, String password) throws IOException, CumstonException {
        String url = MessageFormat.format(identyURL + "?userAccount={0}&password={1}", URLEncoder.encode(userName, "UTF-8"),
                URLEncoder.encode(password, "UTF-8"));
//        HttpGet httpGet = new HttpGet(url);
        HttpPost httpPost = new HttpPost(url);
        CloseableHttpResponse closeableHttpResponse = httpUtil.doPost(httpPost);
        String result = new String();
        if (200 == closeableHttpResponse.getStatusLine().getStatusCode()) {
            return JSON.parseObject(EntityUtils.toString(closeableHttpResponse.getEntity(), "utf-8"), AccountTo.class);
        }
        httpPost.abort();
        throw new CumstonException("identification server error");
    }
